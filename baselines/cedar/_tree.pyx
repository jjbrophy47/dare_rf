...
cdef class _Tree:
    ...
    cpdef int get_exact_node_count(self, double lmbda, int topd, int min_support):
        ...
        """
        return self._get_exact_node_count(self.root, lmbda, topd, min_support)
    ...
    cpdef int get_semi_random_node_count(self, double lmbda, int topd, int min_support):
        ...
        """
        return self._get_semi_random_node_count(self.root, lmbda, topd, min_support)
    ...
    cpdef void print_node_type_count(self, double lmbda, int topd, int min_support):
        ...
        top d layers.
        """
        cdef int exact_node_count = self._get_exact_node_count(self.root, lmbda, topd, min_support)
        cdef int semi_random_node_count = self._get_semi_random_node_count(self.root, lmbda, topd, min_support)
        printf('no. exact: %d, no. semi-random: %d\n', exact_node_count, semi_random_node_count)
    ...
    cdef int _get_exact_node_count(self, Node* node, double lmbda,
        ...
        if not node or node.is_leaf:
            ...
            if lmbda < 0 or (node.depth < topd and node.count <= min_support):
                result = 1

            result += self._get_exact_node_count(node.left, lmbda, topd, min_support)
            result += self._get_exact_node_count(node.right, lmbda, topd, min_support)

        return result

    cdef int _get_semi_random_node_count(self, Node* node, double
                                         lmbda, int topd, int min_support) nogil:
        cdef int result = 0
        ...
        if not node or node.is_leaf:
            ...
            if node.depth < topd and node.count >= min_support and lmbda >= 0:
                result = 1

            result += self._get_semi_random_node_count(node.left, lmbda, topd, min_support)
            result += self._get_semi_random_node_count(node.right, lmbda, topd, min_support)

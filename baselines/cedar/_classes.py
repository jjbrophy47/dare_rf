...
-The nodes in the top d layers that meet the minimum
 sample support use semi-random splits, all other nodes
 use exact unlearning.

-Budget is split among the top d layers.

-Special cases:

  * When retraining a decision node in a given layer, if the
    same feature gets resampled or if it has had no deletions,
    then no retraining is done for that node and it's divergence
    is reset if not already zero.
...
class Tree(object):
    ...
    def get_node_statistics(self):
        ...
        n_exact_nodes = self.tree_.get_exact_node_count(self.lmbda, self.topd_,
                                                        self.min_support)
        n_semi_random_nodes = self.tree_.get_semi_random_node_count(self.lmbda, self.topd_,
                                                                    self.min_support)

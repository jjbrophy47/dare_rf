...
cdef class _Remover:
        ...
        if is_bottom_leaf:
            ...
            # decision node
                ...
                # retrain
                    ...
                    self._add_removal_type(result, node.depth)
                    self._retrain(&node_ptr, X, y, samples, n_samples, &split)
    ...
    cdef int _check_node(self, Node* node, int** X, int* y,
        ...
        if updated_pos_count > 0 and updated_pos_count < updated_count:
            ...
            # TODO (DONE): still need to multiply by probability of parent? I don't think so.
            # p = parent_p * distribution[chosen_ndx]

            max_local_divergence = find_max_divergence(node.sspd, distribution, node.features_count)

            # TODO: If maximum local divergence exceeds this node's budgest, retrain;
            #       otherwise, partition samples to be removed based on the chosen feature.

            # ratio = p / node.p

            # if exp(-epsilon) <= ratio and ratio <= exp(epsilon):

            if max_local_divergence <= node.budget:

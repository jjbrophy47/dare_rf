...
# TODO: do we define max divergence as the max divergence between
#       ANY feature ratio? OR is the max divergence the sum of
#       divergences between all feature ratios?
...
cdef double find_max_divergence(double* sspd1, double* sspd2,
    ...
    for i in range(n_features):
        divergence = fabs(1.0 - (sspd1[i] / sspd2[i]))
        if divergence > max_divergence:
            max_divergence = divergence

    return max_divergence

# TODO: include denominator constant in SSPD calculation?
# TODO: include log_2 n term if using entropy?
@cython.boundscheck(False)
@cython.wraparound(False)
@cython.cdivision(True)
cdef int generate_distribution(double lmbda, double** distribution_ptr,
    ...
    if lmbda < 0 or exp(- lmbda * n_samples * min_score) == 0:
        for i in range(n_scores):
            distribution[i] = 0

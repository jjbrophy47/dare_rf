"""
TODO: assign any probabilities lower than a fixed threshold the value
      of the fixed threhsold. e.g. If thresh = 0.001, then the
      probability 0.000007 => 0.001.
"""
...
cdef double find_max_divergence(double* sspd1, double* sspd2,
    ...
    for i in range(n_features):
        ...
        # printf('sspd1[%d]=%.15f, sspd2[%d]=%.15f, divergence=%.15f\n', i, sspd1[i], i, sspd2[i], divergence)
...
cdef int generate_distribution(double lmbda, double** distribution_ptr,
    ...
    cdef double normalizing_constant2 = 0
    ...
    # printf('min score: %.15f, n_samples: %d, lmbda=%.2f\n', min_score, n_samples, lmbda)
    ...
    # generate probability distribution over the features
        ...
        # printf('lmbda=%.15f, n_samples=%d, multiplier=%.15f\n', lmbda, n_samples, multiplier)
        # printf('scores[67]=%.20f, dist[67]=%e, norm=%e\n', scores[67], distribution[67], normalizing_constant)

        # normalize and clip
        for i in range(n_scores):
            distribution[i] /= normalizing_constant

            # clip probabilities from getting too small
            if distribution[i] < atol:
                distribution[i] = atol

            normalizing_constant2 += distribution[i]

        # renormalize
        for i in range(n_scores):
            distribution[i] /= normalizing_constant2

        # printf('norm_dist[67]=%e\n', distribution[67])

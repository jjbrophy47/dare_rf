...
cdef class _Adder:
    ...
    cdef void _add(self, Node** node_ptr, int** X, int* y,
        ...
        # middle node
            ...
            if result == 1:
                # printf('[A] convert leaf to decision node\n')
                self._add_add_type(result, node.depth)
    ...
    cdef void _retrain(self, Node*** node_pp, int** X, int* y, int* samples,
        ...
        # printf('[R] node.count=%d, n_samples=%d, leaf_samples_count=%d\n', node.count, n_samples, leaf_samples_count)

...
cdef class _Remover:
    ...
    cdef void get_leaf_samples(self,
        ...
        # decision node
            ...
            # traverse left
            if node.left:
                self.get_leaf_samples(node.left, remove_samples, leaf_samples)

            # traverse right
            if node.right:
                self.get_leaf_samples(node.right, remove_samples, leaf_samples)
    ...
    cdef SIZE_t update_greedy_node_metadata(self,
        ...
        for j in range(node.n_features):
            ...
            if n_invalid_thresholds > 0:
                ...
                # possibly other thresholds, sample new thresholds
                 else:

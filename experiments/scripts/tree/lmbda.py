...
def vary_lmbda(args, logger, out_dir, seed):
    ...
    start = time.time()
    d_tree = cedar.Tree(lmbda=100000, max_depth=args.max_depth, verbose=args.verbose, random_state=seed)
    ...
    if args.save_results:
        proba = d_tree.predict_proba(X_test)[:, 1]
    ...
    for i, lmbda in enumerate(lmbdas):
        ...
        start = time.time()
        dt_tree = cedar.Tree(lmbda=lmbda, max_depth=args.max_depth, verbose=args.verbose, random_state=seed)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_dir', type=str, default='output/tree/lmbda', help='output directory.')

...
def remove_sample(args, logger, out_dir, seed):
    ...
    start = time.time()
    tree = cedar.Tree(epsilon=args.epsilon, lmbda=args.lmbda, max_depth=args.max_depth,
                      verbose=args.verbose, random_state=seed)
    tree = tree.fit(X_train, y_train)
    end_time = time.time() - start
    ...
    if args.save_results:
        np.save(os.path.join(out_dir, 'cedar_ep{}_time.npy'.format(args.epsilon)), delete_times)
        np.save(os.path.join(out_dir, 'cedar_ep{}_type.npy'.format(args.epsilon)), delete_types)

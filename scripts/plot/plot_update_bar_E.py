"""
Plot update results as four clustered bar graphs in two-columns:
    1) Sub-1 efficiency
...
def main(args):
    ...
    fig, axs = plt.subplots(3, 1, figsize=(width, height * 2.5), sharex=True)

    tol_list = ['0.1%', '0.25%', '0.5%', '1.0%']
    labels = ['D-DART'] + ['R-DART (tol={})'.format(tol) for tol in tol_list]
    colors = ['0.0', '1.0', '0.75', '0.5', '0.25']

    titles = ['Efficiency Using Random',
              'Efficiency Using Worst-of-{}',
              'Efficiency Difference Between Random and Worst-of-{}',
              'Test Error Increase Relative to D-DART']
    ...
    sub1_n_model_yerr = np.reshape(sub1_n_model_std, (5, 2, sub1_n_datasets), order='F')
    subX_n_model_yerr = np.reshape(subX_n_model_std, (5, 2, sub1_n_datasets), order='F')
    metric_diff_yerr = np.reshape(metric_diff_std_list, (4, 2, sub1_n_datasets), order='F')
    ...
    leg = axs[0].legend(labels=labels, ncol=2, framealpha=1.0, loc='upper right')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, nargs='+', help='datasets to use for plotting',
                        default=['surgical', 'vaccine', 'adult', 'bank_marketing'])
    parser.add_argument('--in_dir', type=str, default='output/', help='input directory.')
    parser.add_argument('--out_dir', type=str, default='output/plots/update_bar_E/', help='output directory.')

    parser.add_argument('--criterion', type=str, default='entropy', help='split criterion.')

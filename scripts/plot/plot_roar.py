...
def main(args):
    ...
    for i, metric in enumerate(['acc', 'auc']):
        ...
        for method in args.method:
            ...
            percentage = str_to_np(temp['percentage'].iloc[0])
            performance = str_to_np(temp.iloc[0][metric])
            performance_std = str_to_np(temp.iloc[0]['{}_std'.format(metric)])
            # ax.errorbar(percentage, performance, yerr=performance_std, label=method.capitalize())
            ax.errorbar(percentage, performance, label=method.capitalize())

        ax.set_title(metric)
        ax.set_xlabel('% train data deleted')
